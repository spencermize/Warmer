	...
	L.Map.addInitHook(function(){
		this.whenReady(function(){
			...
			if (!('exportError' in this)){
				this.exportError = {
					wrongBeginSelector: 'wrong begin selector',
					wrongEndSelector: 'wrong end selector',
					jqueryNotAvailable: 'no jquery',
					popupWindowBlocked: 'popup window blocked',
			...
			this.export = function(options){
					...
					case 'string':
						...
						case '$':
							...
							if (typeof jQuery !== 'undefined'){
								elements = $(jQuerySelector,container);
				...
				for (var i2 = 0; i2 < hide.length; i2++){
					hide[i2].setAttribute('data-html2canvas-ignore','true');
				}
